...
tPilha* insere_pilha(tPilha *pilha, int valor);
...
void gera_lab(int px, int py, char** lab)
{
    ...
    int i, j, passo, k=0, ir, caminho;

    lab[px][py] = '0';///posição = 0
    caminho = rand()%4;///sorteia para onde ir
    while(k!=4)///verifica se tem lado pra ir
    {
        if(caminho == 0 && px-2>=1 && lab[px-2][py] =='1')///Cima
        {
            lab[px-1][py] = '0';
            gera_lab(px-2, py, lab);
        }
        if(caminho == 1 && py+2<c-1 && lab[px][py+2] == '1')///Direita
        {
            lab[px][py+1] = '0';
            gera_lab(px, py+2, lab);
        }
        if(caminho == 2 && px+2<l-1 && lab[px+2][py] == '1')///Baixo
        {
            lab[px+1][py] = '0';
            gera_lab(px+2, py, lab);
        }
        if(py-2>=1 && lab[px][py-2] == '1')///Esquerda
        {
            lab[px][py-1] = '0';
            gera_lab(px, py-2, lab);
        }
...
void solucao(int px, int py,char** lab)
{
    ...
    {
        lab[px][py] = 'x';
        if(px-1>=0&&(lab[px-1][py] == '0' ||lab[px-1][py] == 'S') && achei == 0)///cima
            solucao(px-1, py, lab);
        if(py+1<c&&(lab[px][py+1] == '0'||lab[px][py+1] == 'S') && achei == 0)///direita
            solucao(px, py+1, lab);
        if(px+1<l&&(lab[px+1][py] == '0'||lab[px+1][py] == 'S') && achei == 0)///baixo
            solucao(px+1, py, lab);
        if (py-1>=0&&(lab[px][py-1] == '0'||lab[px][py-1] == 'S') && achei == 0)///esquerda
            solucao(px, py-1, lab);
        if(achei==0)
        {
            lab[px][py] = '0';
...
char** carregaLabirinto(char** lab)
{
    ...
    //printf("Entre com o numero do labirinto: ");
    //scanf("\n%d", &num_lab);
    FILE *arquivo;
